...
class ExpenseSummaryStatus(APIView):
    ...
    def get(self, request):
        ...
        ayear_age = todays_date - datetime.timedelta(days=30*12)
        expenses = Expenses.objects.filter(owner=request.user, date__gte=ayear_age, date__lte=todays_date)
...
class IncomeSummaryStatus(APIView):
    ...
    def get(self, request):
        todays_date = datetime.date.today()
        ayear_age = todays_date - datetime.timedelta(days=30*12)
        income = Income.objects.filter(owner=request.user, date__gte=ayear_age, date__lte=todays_date)
